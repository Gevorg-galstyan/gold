...
@section('content')
    ...
    <div style="position:relative; z-index:9; text-align:center;">
        ...
        <a href="{{ route('profile.edit', ['slug' => Auth::user()->slug,'changes' => 'edit-personal-data']) }}" class="btn btn-primary">
            {{ __('profile.edit_personal_data') }}
        </a>

        <a href="{{ route('profile.edit', ['slug' => Auth::user()->slug,'changes' => 'edit-login']) }}" class="btn btn-primary">
            {{ __('profile.edit_login') }}
        </a>

        <a href="{{ route('profile.edit', ['slug' => Auth::user()->slug,'changes' => 'edit-password']) }}" class="btn btn-primary">
            {{ __('profile.edit_password') }}
        </a>
    </div>
@stop
