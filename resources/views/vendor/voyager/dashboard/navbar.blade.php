            ...
            @section('breadcrumbs')
                <ol class="breadcrumb hidden-xs">
                    ...
                    @for($i = 1; $i <= count(Request::segments()); $i++)
                        ...
                        @if(Request::segment($i) != ltrim(route('voyager.dashboard', [], false), '/') &&
                        !is_numeric(Request::segment($i))&&
                        !in_array(Request::segment($i),config('voyager.multilingual.locales')))
                ...
                <ul class="nav navbar-nav">
                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button"
                           aria-expanded="false"><img id="imgNavSel"
                                                      src="{{asset('storage/'.LaravelLocalization::getCurrentLocaleScript())}}"
                                                      alt="..." class="img-thumbnail icon-small">  <span
                                    id="lanNavSel">{{LaravelLocalization::getCurrentLocaleName()}}</span> <span
                                    class="caret"></span></a>
                        <ul class="dropdown-menu" role="menu">
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                <li>
                                    <a id="navIta" hreflang="{{ $localeCode }}"
                                       href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}"
                                       class="language">
                                        <img id="imgNavIta" src="{{asset('storage/'.$properties['script'])}}" alt="..."
                                             class="img-thumbnail icon-small"><span
                                                id="lanNavIta">{{ $properties['name'] }}</span></a>
                                </li>
                            @endforeach
                        </ul>
                    </li>
        ...
        <ul class="nav navbar-nav @if (config('voyager.multilingual.rtl')) navbar-left @else navbar-right @endif">
            <li class="dropdown profile">
                ...
                <ul class="dropdown-menu dropdown-menu-animated">
                    ...
                    <?php $nav_items = config('voyager.' . (Auth::user()->role->id == 1 ? 'dashboard' : 'user_dashboard') . '.navbar_items'); ?>
